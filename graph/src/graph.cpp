...
void Graph::render(SDL_Renderer *rend, SDL_Rect r, const std::function<float(float)> &func) const
{
    ...
    {
        float x = r.x + 20 + (p.x / m_xmax) * (r.w - 40);
        float y = r.y + 20 + (1.f - p.y / m_ymax) * (r.h - 40);
    ...
    {
        float x1 = (float)(x - (r.x + 20)) / (r.w - 40) * m_xmax;
        float x2 = (float)(x + 1 - (r.x + 20)) / (r.w - 40) * m_xmax;
        float y1 = r.y + r.h - 20 - (func(x1) / m_ymax * (r.h - 40));
        float y2 = r.y + r.h - 20 - (func(x2) / m_ymax * (r.h - 40));
