...
The output of $g$ can be interpreted as the "probability" that class is 1.

ex. $g(z) = 0.7$ means there is a 70\% chance $y$ is 1
...
\[ J(\vec{w},b) = \frac{1}{m} \sum_{i=1}^{m} L(g_z,\vec{y}_i) \]
...
\begin{align*}
    Z^{[\ell]}_i &= \vec{W}^{[\ell]}_i \vec{x} + \vec{b}^{[\ell]}\\
    A^{[\ell]} &= g(Z^{[\ell]})
\end{align*}
