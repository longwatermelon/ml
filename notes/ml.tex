...
$\vec{x}^{(i)}$ = feature vector for item $i$

For user $j$, predict rating of item $i$ with $\vec{w}^{(j)} \cdot \vec{x}^{(i)} + b^{(j)}$
...
\begin{center}
    \begin{tabular}{ ||c|c|c|c|| }
        \hline
        Movie & $x_1$ (Romance) & $x_2$ (Action) & $x_3$ (Horror)\\
        \hline
        Romance movie & 1.0 & 0.1 & 0.0\\
        Action movie & 0.0 & 1.0 & 0.0\\
        Comedy movie & 0.5 & 0.0 & 0.0\\
        Horror movie & 0.0 & 1.0 & 1.0\\
        \hline
    \end{tabular}
\end{center}
...
\[ J(\vec{w}^{(j)},b^{(j)}) = \frac{1}{2m^{(j)}} \sum_{i:r(i,j)=1} (\vec{w}^{(j)} \cdot \vec{x}^{(i)} + b^{(j)} - y^{(i,j)})^2 \]
...
\[ J(W,\vec{b}) = \frac{1}{2} \sum_{j=1}^{n_u} \left[ \sum_{i:r(i,j)=1} (\vec{w}^{(j)} \cdot \vec{x}^{(i)} + b^{(j)} - y^{(i,j)})^2 \right] \]
