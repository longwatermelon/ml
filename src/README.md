...
`fnn-mnist.cpp`: A simple FNN trained on an MNIST digits dataset.
...
`cnn-mnist.cpp`: A simple CNN trained on an MNIST digits dataset.
  * Data comes in as `[N,1,28,28]` where `N` is batch size. The network is `conv5 Cout=4` ---> `cont5 Cout=8` ---> flatten ---> `64` linear ---> `10` linear.

`shakespeare-gpt.cpp`: Trains a transformer to generate Shakespeare-like text.

`tinystories.cpp`: Trains a transformer to generate simple stories.
